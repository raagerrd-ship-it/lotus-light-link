...
const Index = () => {
  ...
  return (
    ...
    >
      ...
      {activated && (
        <div className="absolute inset-0 transition-[top,bottom] duration-300" style={{ top: showOverlay && (connected || !busy) ? '2.75rem' : 0, bottom: showCalibration ? '16rem' : (showOverlay && (nowPlaying?.trackName || nowPlaying?.isTvMode) && nowPlaying.playbackState !== "PLAYBACK_STATE_IDLE" ? '4.5rem' : 0) }}>
          <MicPanel char={firstChar} currentColor={currentColor} palette={currentPalette} sonosVolume={nowPlaying?.volume} isPlaying={!!(nowPlaying?.trackName || nowPlaying?.isTvMode) && nowPlaying?.playbackState === "PLAYBACK_STATE_PLAYING"} trackName={nowPlaying?.isTvMode ? 'TV' : (nowPlaying?.trackName ?? null)} tickMs={tickMs} chartEnabled={chartEnabled} onLiveStatus={handleLiveStatus} />
        </div>
      )}
