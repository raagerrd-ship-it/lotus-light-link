...
function MicGainCalibrateButton({ piBase, onDone }: { piBase: string; onDone: (gain: number) => void }) {
  const [running, setRunning] = useState(false);
  const [elapsed, setElapsed] = useState(0);
  const [duration, setDuration] = useState(15000);
  const [result, setResult] = useState<null | { ok: boolean; measuredRms: number; newGain: number; oldGain: number; targetRms: number }>(null);
  ...
  useEffect(() => {
    ...
    const id = setInterval(async () => {
      try {
        ...
        if (r.active) {
          setElapsed(r.elapsedMs || 0);
          setDuration(r.durationMs || 15000);
        } else {
          setRunning(false);
          if (r.lastResult) {
            setResult(r.lastResult);
            if (r.lastResult.ok) onDone(r.lastResult.newGain);
          }
        }
      } catch { /* keep polling */ }
    }, 500);
    return () => clearInterval(id);
  }, [running, piBase, onDone]);
  ...
  const pct = running ? Math.min(100, Math.round((elapsed / duration) * 100)) : 0;
  ...
  return (
    <div className="mt-3">
      ...
      >
        {running ? `Kalibrerar… ${pct}%` : 'Kalibrera mic-gain (15s)'}
      </button>
      {running && (
      ...
      {result && !running && (
        <p className="text-[10px] text-muted-foreground mt-1">
          {result.ok
            ? `RMS ${result.measuredRms.toFixed(3)} → gain ${result.oldGain.toFixed(1)}× → ${result.newGain.toFixed(1)}× (target ${result.targetRms.toFixed(2)})`
            : `Kalibrering misslyckades (för tyst? rms=${result.measuredRms.toExponential(1)}).`}
        </p>
      )}
      <p className="text-[10px] text-muted-foreground mt-1">
        Spela musik på normal-volym och tryck — motorn justerar base-gain så att effektiv RMS ≈ 0.35.
      </p>
    </div>
  );
}
