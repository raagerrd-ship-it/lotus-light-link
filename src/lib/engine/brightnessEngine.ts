...
/** Fast version using precomputed alphas — zero Math.pow. */
export function smoothFast(prev: number, raw: number, preAttack: number, preRelease: number): number {
  const alpha = raw > prev ? preAttack : preRelease;
  return prev + alpha * (raw - prev);
}
...
/** Fast version using precomputed alpha — zero Math.pow/exp. */
export function extraSmoothFast(prev: number, newVal: number, preAlpha: number): number {
  return prev + (1 - preAlpha) * (newVal - prev);
}
...
export function perceptualBrightness(pct: number, floor: number = 0): number {
  ...
  return floor + perceived * (100 - floor);
}
...
// Reusable result object for fast path (zero-alloc)
const _bResult = { pct: 0, newCenter: 0 };

/** Fast version using precomputed centerAlpha — eliminates Math.pow. */
export function computeBrightnessPctFast(
  bassNorm: number,
  midHiNorm: number,
  effectiveMax: number,
  dynamicCenter: number,
  ...
  const newCenter = dynamicCenter + (energyNorm - dynamicCenter) * tc.centerAlpha;
  energyNorm = applyDynamics(energyNorm, newCenter, cal.dynamicDamping);
  ...
  _bResult.pct = Math.max(floor, pct);
  _bResult.newCenter = newCenter;
  return _bResult;
}
