...
import { getAlsaDevice, setAlsaDevice, getMicGain, setMicGain, getEffectiveGain, getAutoGainMultiplier, disableAutoGain, setAutoGainFromVolume } from './alsaMic.js';
...
export function startConfigServer(engine: PiLightEngine, port = 3001): void {
  ...
  app.put('/api/mic-gain', (req, res) => {
   ...
   app.get('/api/auto-gain', (_req, res) => {
     res.json({ enabled: autoGainEnabled, multiplier: getAutoGainMultiplier(), effective: getEffectiveGain() });
   });
   app.put('/api/auto-gain', (req, res) => {
     const { enabled } = req.body;
     if (typeof enabled === 'boolean') {
       autoGainEnabled = enabled;
       if (!enabled) disableAutoGain();
       res.json({ ok: true, enabled, multiplier: getAutoGainMultiplier(), effective: getEffectiveGain() });
     } else {
  ...
  app.get('/api/diagnostics', (_req, res) => {
    ...
    res.json({
      ...
      ranges: {
        ...
        bassNorm:      { ok: [0.1, 0.9],   warn: '>0.95 = AGC trackar för tätt' },
        midHiNorm:     { ok: [0.1, 0.9],   warn: '>0.95 = AGC trackar för tätt' },
        bassMax:       { ok: [0.005, 1.0], warn: 'AGC bas-tak' },
        midHiMax:      { ok: [0.005, 1.0], warn: 'AGC disk-tak' },
