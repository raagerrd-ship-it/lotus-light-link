...
export class Analyser {
  ...
  private sectionHop(hops: number, intSum: number, kicks: number, breaking: boolean, nowMs: number, dtMs: number, rms2Sum: number, centSum: number, spec: ArrayLike<number>, specOff = 0): void {
    ...
    if (Analyser.SECTION_MODE === 'rank') {
      ...
      const db = 10 * Math.log10(this.secBlkRms2 / n + 1e-10); this.secBlkDb.push(db); this.secBlkDens.push(bKicks); if (Analyser.REPEAT & 1) this.secBlkT.push(nowMs);
      if (this.secBlkDb.length > 600) { this.secBlkDb.shift(); this.secBlkDens.shift(); if (Analyser.REPEAT & 1) this.secBlkT.shift(); }
