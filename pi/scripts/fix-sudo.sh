...
#
# Standalone version av pre-flight blocket i pi/setup-lotus.sh. Kör fristående
# när sudo är trasigt utan att behöva köra hela installationen.
#
# Anropas av:
#   - pi/setup-lotus.sh (steg 0)
#   - manuellt: bash /opt/lotus-light/pi/scripts/fix-sudo.sh
#
# Försöker reparera som root direkt, eller via pkexec om vi är vanlig användare.
# Skriver tydlig manuell fallback om båda misslyckas.
#
# Exit-koder:
