...
| `LOTUS_PLL_RING` | 1 | Taktklockans PLL fasar mot kick-**ringen** (analysatorns sub-hop-stämpel för varje slag, ±1,3 ms) i stället för tickens `Date.now()`. Bakgrund 2026-09-18: `frame.kickAtMs` är nollskild på en hop per slag, så ticken (1 av 7–9 hoppar) såg den nästan aldrig och föll tillbaka på onset-detektorn på bandtakt + tickkvantisering. Uppmätt (histogram ring-kick mot grid, 143 kickar, 134 BPM, 9 hop): gridet **52 ms sent, IQR 42 ms** — och `beatLeadMs` (132 = 87 ms stigtid + 45 ms utlatens) kompenserade det aldrig. `0` = gamla vägen (A/B). Mätskript: `scratchpad/grid_hist.py` (kräver låt med kick på slaget; 16-dels-bas ger fyra kluster och säger inget). |
