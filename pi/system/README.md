...
| PC-facit ur ljudet (`tools/tempo-facit-pc/`) | på när PC:n kör | **Katalogoberoende facit (2026-09-19).** Vid låtbyte +10 s fångar motorn 30 s @48 kHz (samma ljud som analysatorn, `startRawCapture(…, fullRate=true)`) till `<PCC_DATA_DIR>/snippets/` (kö max 30; hoppar över låtar med facit och manuell råfångst). PC:n **hämtar** (`GET /api/tempo/snippets` → `GET /api/tempo/snippet?key=`), räknar tempo på evidens (se verktygets README: kandidater pinnas, bas-onset på slagen, halvslagstest för oktav — 6/6 på syntetiskt test där librosas default gav 2/6) och skriver `PUT /api/tempo/facit` → cacheraden (`source: pc:…`, `rawBpm`, `candidates`, `pcConf`) + `setMetaTempo` → samma dom vid låtslut → samma oktavledtråd. Första skarpa varvet: "SAME SONGS" PC 106,1 vs analysatorn 106 → ok. Inga portar, ingen PC-adress på Pi:n. `GET /api/tempo/cache` = hela lärdatan. **Händelselogg + dropfångst (samma dag):** `<id>.events.json` (kick-ring, gridpulser, ljus 10 Hz, flaggor) bredvid WAV:en; `drop`-fångst = 15 s förbuffert (`enablePreroll`, 1,44 MB) + 15 s, utlöst av `dropCount` eller `POST /api/tempo/capture`; PC:n svarar med `pc {phase, level, onset, drop, descr}` / `dropEvents`. Första mätningen: kick −10 ms, puls −69 ms (grid ~60 ms sent), onset recall 0,39. |
| `useMetaTempo` (kal) | **av = FACIT-läge** | **Katalogtempo (2026-09-18, `tempoLookup.ts`) — facit för att lära analysatorn, inte styrning (användarens beslut samma kväll).** Med av: uppslag + dom mot analysatorn loggas per låt, men gridet går på analysatorn; per låt samlar motorn analysatorns bpm/konfidens (1 Hz) och kick-ringens intervallhistogram (var 5 s) och skriver vid låtbyte `learn {bpmMedian/Min/Max, confMedian, ringIntervalMs, ringRegular, ringN, ringPerBeat, durationS}` i `tempo-cache.json` bredvid facit-tempot (`[tempo] inlärning:`-rad). `ringPerBeat` 4 = fyra regelbundna transienter per analysatorslag ("Ego": analysatorn 86, ringen 172 ms ⇒ 172). Med `true` driver katalogen gridet (testläge). Mekanik: Vid låtbyte slås artist+titel (Sonos) upp i Deezers publika API (ingen nyckel, ~0,8 s från Pi:n): sök, ranka på titel+artist-likhet (Dice på bigram, `(Radio Edit)`/`feat.` bortskalat), hämta `bpm` för de bästa tills en har bpm>0. Motorn accepterar bara ett tempo som stämmer med analysatorn: kvot ½/1/2 (fönstret [80,160) viker oktaver — "Ego" är 172, analysatorn sa 86) eller ⅔/1,5 (analysatorns fantomer), ±5 %; annars `avvisat`. Tills analysatorn är säker (conf ≥0,6) gäller katalogen provisoriskt. Katalogens oktav viks utanför [70, 180] (Deezer ger "Dancing Queen" 200,7 → 100,4; "Ego" 172 stannar), råvärdet sparas i cachen. Status: `analyser.beatMode = 'katalog'` + `analyser.metaTempo {bpm, source, ratio, verdict, drives}`. Cache `tempo-cache.json` i PCC_DATA_DIR per låtnyckel (`songKey`): träff, negativt svar (7 dagar; nätfel cachas inte) och motorns dom + analysatorns värde = lärdata per låt. **Täckning:** Deezer har bpm 0 för många spår (Emil Assergård, Osman …) — källa 2 (GetSongBPM, kräver gratis nyckel + attribution) är nästa steg; katalogfri oktavregel ur kick-ringen (fyra regelbundna transienter per gridslag ⇒ 2×) är komplementet. Låtminnet (`useRecording`) går före katalogen när det har en träff. |
